from circumat.tasks import execute_calc
from circumat.models import Job
from celery.contrib.testing.worker import start_worker
from django.test import SimpleTestCase
from circumatMasterProject.celery import app
...
class TestAjaxCelery(SimpleTestCase):
    """
    This class tests Ajax and Celery
    test_celery_default = tests the default query
    test_celery_bounds = looks for outer boundaries of indicators
    ...
    def test_celery_default(self):
        self.task = execute_calc.apply(
            args=[self.job_id, 1, "channel_name", {
                'dimType': 'Production',
                'vizType': 'TreeMap',
        ...
        models.TaskResult.objects.create(
            task_id=self.task.id, result=self.task.get())
    ...
    def test_celery_bounds(self):
        ...
        # store the results manually
        models.TaskResult.objects.create(task_id=self.task.id,
